...
public class Block extends Entity implements RayCastCallback, Disposable {
	...
	protected void createBody(float bodySpawnX, float bodySpawnY, BodyType bodyType, short categoryBits, short maskBits) {
		...
		float[] vertices = new float[] {-8, 8,
										...
										8, -8,
										-8, -8,
										-8, 8}; // We could create a polygonal box but this way player box body does not randomly get stuck bodyedges.
...
//		}
	...
	public float reportRayFixture(Fixture fixture, Vector2 point, Vector2 normal, float fraction) {
		if (fixture.getFilterData().categoryBits == CollisionContactListener.BLOCK_COL && fixture.getBody() != this.body) {
			hit = true;
		} else if (fixture.getFilterData().categoryBits == CollisionContactListener.TERRAIN_COL && fixture.getBody() != this.body) {
			hit = true;
		}
		return -1;
	}
	...
	public void tick(float delta) {
		...
		if (spriteEdgeU != null)
			spriteEdgeU.setPosition(body.getTransform().getPosition().x - spriteEdgeU.getRegionWidth() / 2, body.getTransform().getPosition().y - spriteEdgeU.getRegionHeight() / 2);
		if (spriteEdgeD != null)
			spriteEdgeD.setPosition(body.getTransform().getPosition().x - spriteEdgeD.getRegionWidth() / 2, body.getTransform().getPosition().y - spriteEdgeD.getRegionHeight() / 2);
		if (spriteEdgeL != null)
			spriteEdgeL.setPosition(body.getTransform().getPosition().x - spriteEdgeL.getRegionWidth() / 2, body.getTransform().getPosition().y - spriteEdgeL.getRegionHeight() / 2);
		if (spriteEdgeR != null)
			spriteEdgeR.setPosition(body.getTransform().getPosition().x - spriteEdgeR.getRegionWidth() / 2, body.getTransform().getPosition().y - spriteEdgeR.getRegionHeight() / 2);
	...
	public void render(float delta) {
		...
		super.bodyDimensions.set(new Vector3(16 + super.bBExtra / GameScreen.PPM, 16 + super.bBExtra / GameScreen.PPM, 0));
