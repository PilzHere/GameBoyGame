 ...
 *
 */
public class Ant extends Entity implements Disposable, RayCastCallback {
	...
	public Ant(GameScreen screen, float spawnPosX, float spawnPosY) {
		super.setScreen(screen);
//		super.id = screen.enemies.size;
		
		texRegIdleLeft = new TextureRegion(AssetsManager.MANAGER.get(AssetsManager.SPIDER, Texture.class), 1, 25, 40, 15);
	...
	private void setupSpriteWalkingLeft(Texture tex) {
		TextureRegion texRegWalkingLeft1 = new TextureRegion(tex, 1, 25, 40, 15);
		TextureRegion texRegWalkingLeft2 = new TextureRegion(tex, 41, 25, 40, 15);
		TextureRegion texRegWalkingLeft3 = new TextureRegion(tex, 81, 25, 40, 15);
...
//		System.err.println("Spider FractL: " + tempFractLeft + "    FractR: " + tempFractRight + "    FacingLeft: " + facingLeft);
	...
	public void tick(float delta) {
		...
		spriteIdleLeft.setPosition(body.getTransform().getPosition().x - spriteIdleLeft.getRegionWidth() / 2, body.getTransform().getPosition().y + 2.99f - spriteIdleLeft.getRegionHeight() / 2);
		spriteWalkingLeft.setPosition(body.getTransform().getPosition().x - spriteWalkingLeft.getRegionWidth() / 2, body.getTransform().getPosition().y + 2.99f - spriteWalkingLeft.getRegionHeight() / 2);
	}
