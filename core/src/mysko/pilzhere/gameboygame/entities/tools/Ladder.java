...
public class Ladder extends Entity implements Disposable {
	...
	public Ladder(GameScreen screen, float spawnPosX, float spawnPosY) {
		...
		texRegLadderVertical = new TextureRegion(AssetsManager.MANAGER.get(AssetsManager.ATLAS01, Texture.class), 110, 2, 16, 16);
	...
	public void tick(float delta) {
		spriteLadderVertical.setPosition(body.getTransform().getPosition().x - spriteLadderVertical.getRegionWidth() / 2, body.getTransform().getPosition().y - spriteLadderVertical.getRegionHeight() / 2);
	}
