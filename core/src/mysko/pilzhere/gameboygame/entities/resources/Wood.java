 ...
 *
 */
public class Wood extends Resource implements Disposable {
	
	public Wood(GameScreen screen, float spawnPosX, float spawnPosY) {
		super(screen, spawnPosX, spawnPosY);
		
		createBody(spawnPosX, spawnPosY);
		
		super.texRegMain = new TextureRegion(AssetsManager.MANAGER.get(AssetsManager.ATLAS01, Texture.class), 74, 20, 16, 16);
...
//		super.body.setGravityScale(2);
	...
	public void tick(float delta) {
		...
		super.spriteMain.setPosition(super.body.getTransform().getPosition().x - super.spriteMain.getRegionWidth() / 2, super.body.getTransform().getPosition().y - 0.99f - super.spriteMain.getRegionHeight() / 2);
	}
