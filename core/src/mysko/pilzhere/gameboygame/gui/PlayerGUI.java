...
//		If uneven: keep last heart half.
	...
	public void render(float delta) {
		screen.shapeRenderer.setProjectionMatrix(screen.shapeRenderer.getProjectionMatrix().setToOrtho2D(0, 0, GameboyGame.GAMEBOY_WINDOW_WIDTH, GameboyGame.GAMEBOY_WINDOW_HEIGHT));
		...
		screen.game.batch.setProjectionMatrix(screen.game.batch.getProjectionMatrix().setToOrtho2D(0, 0, GameboyGame.GAMEBOY_WINDOW_WIDTH, GameboyGame.GAMEBOY_WINDOW_HEIGHT));
		...
		if (GameboyGame.USING_PROFILER) {
			...
			gbFont.draw(screen.game.batch, "TextureBindings: " + GameboyGame.profiler.getTextureBindings(), 1, 144-8-8-8);
			gbFont.draw(screen.game.batch, "ShaderSwitches: " + GameboyGame.profiler.getShaderSwitches(), 1, 144-8-8-8-8);
			gbFont.draw(screen.game.batch, "UsedVertices: " + GameboyGame.profiler.getVertexCount().count, 1, 144-8-8-8-8-8);
			gbFont.draw(screen.game.batch, "ObjectsRendered: " + screen.objectsRendered, 1, 144-8-8-8-8-8-8);
		}
